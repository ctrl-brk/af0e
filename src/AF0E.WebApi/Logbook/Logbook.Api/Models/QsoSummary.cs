...
public sealed class QsoSummary(HrdLog log, bool isAdmin = false)
{
    ...
    public string? StationCallsign { get; set; } = log.ColStationCallsign;
    public string? OperatorCallsign { get; set; } = log.ColOperator;
    public string? Comment { get; set; } = isAdmin ? log.ColComment : null;
