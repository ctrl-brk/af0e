    ...
    private static void GeneratePage(XGraphics gfx, List<LabelData> data, TemplateType templateType, int startLabelNum, bool printDeliveryMethod)
    {
        ...
        foreach (var label in data)
        {
            ...
            if (label.PrintHeader)
            {
                ...
                if (printDeliveryMethod && (label.Delivery == "B" || label.Delivery == "D")) //buro or direct
                {
                ...
                Draw($"AFØE confirms the following {(label.TotalContacts > 1 ? $"{label.TotalContacts} QSOs" : "QSO")}:", _confirmFont);
                curX += 3; curY += 17;

                var header = label.HasPota
                    ? "YY-MM-DD  UTC  Mhz Mode RST POTA     County            "
                    : "Date (YMD)    UTC      Band      Mode      RST";

                if (label.HasPota)
                {
                    if (label.MaxCountyLength > 17)
                    {
                        header += "  ";
                        label.ShortGrid = true;
                    }
                    if (label.MaxCountyLength < 20)
                        header += "Grid";
                    else
                        label.NoGrid = true;
                }
                ...
                curX = startX; curY += 11;
                Draw("", null, label.HasPota ? 272 : 210);
            }
            ...
            foreach (var q in label.Contacts)
            {
                ...
                if (label.HasPota)
                {
                    var grid = label.NoGrid || string.IsNullOrWhiteSpace(q.Parks) ? "" : label.ShortGrid ? q.MyGrid?[..4] : q.MyGrid;
                    var county = q.MyCounty;
                    if (county is { Length: <= 21 })
                        county += $",{q.MyState}";

                    county = label.MaxCountyLength switch
                    {
